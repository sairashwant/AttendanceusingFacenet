...
def main():
    ...
    if uploaded_file is not None:
        ...
        if st.button('Detect Faces'):
            ...
            df = pd.DataFrame(detected_names, columns=['Detected Names'])
            csv_file = 'detected_names.csv'
            df.to_csv(csv_file, index=False)
            st.success(f'Detected names saved to {csv_file}.')
            if os.path.exists('detected_names.csv'):
                st.download_button(label="Download Detected faces.csv", data='detected_names.csv', file_name='Detected faces.csv', mime='text/csv')
